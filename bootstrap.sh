...
#   3. Python               (only needed by sync-md-ts.sh; optional)
#   4. Installs the agent   (copies .agents/teachme.ts -> ~/.agents/)
...
# ─── Step 3: Python (optional, for sync-md-ts.sh) ────────────
ensure_python() {
    if have python || have python3; then
        print_ok "Python available (needed only for sync-md-ts.sh)"
    else
        print_warn "Python not found — sync-md-ts.sh won't work until you install it (optional)"
    fi
}
...
install_agent() {
    if [ ! -f "$SOURCE_FILE" ]; then
        print_error "Agent source not found: $SOURCE_FILE"
        echo "  Run this script from inside the teachme/ folder."
        exit 1
    fi
    mkdir -p "$AGENTS_DIR"
...
do_install() {
    ...
    [ -z "$py" ] && have python3 && py=python3
    if [ -n "$py" ]; then
        local ver
        ver=$("$py" "$SCRIPT_DIR/sync_helper.py" --version 2>/dev/null | grep -o 'v[0-9]\+\.[0-9]\+\.[0-9]\+' | head -1)
...
do_check() {
    ...
    # Sync check (optional; needs Python)
    local py=""
    have python  && py=python
    [ -z "$py" ] && have python3 && py=python3
    if [ -n "$py" ] && [ -f "$SCRIPT_DIR/sync_helper.py" ]; then
        if "$py" "$SCRIPT_DIR/sync_helper.py" --check >/dev/null 2>&1; then
...
do_version() {
    ...
    "$py" "$SCRIPT_DIR/sync_helper.py" --version
}
